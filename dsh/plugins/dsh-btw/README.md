...
用户于2026-09-26确认停止维护此独立自研 `/btw` 插件，改用 Web UI 整合包的侧栏对话。部署配置保持 `dsh-btw` 条目 `disabled: true`；不再安装新版本或为它重启 DSH。源码、测试和历史会话保留。以下0.3.0适配说明仅为归档，不代表线上启用版本。
...
此插件已退役，不执行新的打包部署。历史目录型 `file:` 依赖曾保留旧安装副本，因此不能仅根据源码版本声称已部署；线上保留的0.2.0安装副本现已禁用，0.3.0源码仅作归档。未部署的补升级包和安装校验实验已撤出本仓库，不再作为后续维护计划。
